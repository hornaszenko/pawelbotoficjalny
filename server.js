...
var arguments2, member_xp, command, member_level;


s4d.client.login('NzY4MTkzNzI4NjkwMTI2ODg5.X486KQ.8Z0rAPMyEuZfZlakUDmn9RxoQww').catch((e) => s4d.tokenInvalid = true);
...
s4d.client.on('message', async (s4dmessage) => {
    if (!((s4dmessage.member).user.bot)) {
        ...
        if (member_xp > 100) {
            ...
            s4dmessage.channel.send(String((['Gratulacje,', (s4dmessage.member), 'Wskoczyłeś na poziom', member_level, '!!'].join(''))));
        }
        if ((s4dmessage.content) == 's!level') {
            s4dmessage.channel.send(String(([(s4dmessage.member), ', jesteś obecnie na poziomie ', member_level].join(''))));
...
bot.on("message", (message) => {
  ...
  if (msg.toLowerCase() == "penis") {
   ...
   if (msg.toLowerCase() == "nie ruszam dupy z klopa") {
     message.channel.send("ta zupa była z mlekiem")
  }
  
   if (msg.toLowerCase() == "na mleko mam alergie") {
